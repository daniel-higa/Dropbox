...
{    
    ...
    public function __construct($key)
    {
        if (!extension_loaded('mcrypt')) {
            throw new _Dropbox_Exception('The storage encrypter requires the MCrypt extension');
        } elseif (($length = mb_strlen($key, '8bit')) !== self::KEY_SIZE) {
            throw new _Dropbox_Exception('Expecting a ' .  self::KEY_SIZE . ' byte key, got ' . $length);
        } else {
